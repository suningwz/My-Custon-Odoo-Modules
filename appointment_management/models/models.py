...
class Attendee(models.Model):
    ...
    event_id = fields.Many2one('calendar.schedule', 'Meeting linked', ondelete='cascade')
        ...
        for attendee in self:
            if attendee.email or attendee.partner_id.email:
                ...
                if ics_file:
                    ...
                    mail_ids.append(invitation_template.with_context(no_document=True).send_mail(attendee.id,
...
class AlarmManager(models.AbstractModel):
    ...
    def _get_next_potential_limit_alarm(self, alarm_type, seconds=None, partner_id=None):
        ...
        events = self.env['calendar.schedule'].browse(result)
        result = {
            key: result[key]
    ...
    def do_check_alarm_for_one_date(self, one_date, event, event_maxdelta, in_the_next_X_seconds, alarm_type,
        ...
        if one_date - timedelta(minutes=(missing and 0 or event_maxdelta)) < datetime.datetime.now() + timedelta(
                seconds=in_the_next_X_seconds):  # if an alarm is possible for this date
            for alarm in event.alarm_ids:
                if alarm.alarm_type == alarm_type and \
                        one_date - timedelta(
                    minutes=(missing and 0 or alarm.duration_minutes)) < datetime.datetime.now() + timedelta(
    ...
    def get_next_mail(self):
        ...
        for meeting in self.env['calendar.schedule'].browse(all_meetings):
    ...
    def get_next_notif(self):
        ...
        for event_id in all_meetings:
            ...
            meeting = self.env['calendar.schedule'].browse(event_id)
            if meeting.recurrency:
                b_found = False
                last_found = False
                for one_date in meeting._get_recurrent_date_by_event():
    ...
    def do_mail_reminder(self, alert):
        meeting = self.env['calendar.schedule'].browse(alert['event_id'])
        alarm = self.env['calendar.alarm'].browse(alert['alarm_id'])

        result = False
        if alarm.alarm_type == 'email':
            result = meeting.attendee_ids.filtered(lambda r: r.state != 'declined')._send_mail_to_attendees(
                'calendar.calendar_template_meeting_reminder', force_send=True, force_event_id=meeting)
        return result
...
class Alarm(models.Model):
    ...
    alarm_type = fields.Selection([('notification', 'Notification'), ('email', 'Email')], string='Type', required=True,
                                  default='email')
    ...
    def _onchange_duration_interval(self):
        ...
        display_alarm_type = {key: value for key, value in self._fields['alarm_type']._description_selection(self.env)}[
            self.alarm_type]
...
class Meeting(models.Model):
    ...
    def default_get(self, fields):
        ...
        if 'res_model_id' not in defaults and 'res_model_id' in fields and \
                self.env.context.get('active_model') and self.env.context['active_model'] != 'calendar.schedule':
    ...
    stop_datetime = fields.Datetime('End Datetime', compute='_compute_dates', inverse='_inverse_dates', store=True,
                                    states={'done': [('readonly', True)]}, tracking=True)  # old date_deadline
    ...
    show_as = fields.Selection([('free', 'Free'), ('busy', 'Busy')], 'Show Time as',
    ...
    rrule_type = fields.Selection([
        ...
        ('monthly', 'Months'),
        ('yearly', 'Years')
    ], string='Recurrence', states={'done': [('readonly', True)]},
    ...
    categ_ids = fields.Many2many('calendar.event.type')
    attendee_ids = fields.One2many('calendar.attendee', 'event_id', 'Participant', ondelete='cascad')
    partner_ids = fields.Many2many('res.partner')
    alarm_ids = fields.Many2many('calendar.alarm')
    ...
    def get_recurrent_ids(self, domain, order=None):
        ...
        if self and recurrent_ids:
            ...
            if len(select_fields) > 1:
                query = "SELECT %s FROM calendar_event WHERE id in %%s" % (
                    ", ".join(select_fields))  # could be improved by only taking event with recurency ?
    ...
    def message_subscribe(self, partner_ids=None, channel_ids=None, subtype_ids=None):
        ...
        return super(Meeting, records).message_subscribe(partner_ids=partner_ids, channel_ids=channel_ids,
                                                         subtype_ids=subtype_ids)

    def _message_subscribe(self, partner_ids=None, channel_ids=None, subtype_ids=None, customer_ids=None):
        records = self.browse(get_real_ids(self.ids))
        return super(Meeting, records)._message_subscribe(partner_ids=partner_ids, channel_ids=channel_ids,
    ...
    def _name_search(self, name='', args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        return super(Meeting, self)._name_search(name=name, args=args, operator=operator, limit=limit,
        ...
        for meeting in self:
                ...
                if not values.get('recurrency', True) or not blacklisted:
                    ...
                    if data.get('rrule'):
                        new_ids = meeting.with_context(dont_notify=True).detach_recurring_event(
                            values).ids  # to prevent multiple _notify_next_alarm
            ...
            if values.get('partner_ids', False):
                attendees_create = all_meetings.with_context(
                    dont_notify=True).create_attendees()  # to prevent multiple _notify_next_alarm
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        return super(Meeting, self.with_context(virtual_id=False)).read_group(domain, fields, groupby, offset=offset,
    ...
    def read(self, fields=None, load='_classic_read'):
        ...
        for calendar_id, real_id in select:
            ...
            ls = calendar_id2real_id(calendar_id,
                                     with_date=res and res.get('duration', 0) > 0 and res.get('duration') or 1)
            ...
            if r['privacy'] == 'private':
                for f in r:
                    ...
                    public_fields = list(set(
                        recurrent_fields + ['id', 'active', 'allday', 'start', 'stop', 'display_start', 'display_stop',
    ...
    def unlink(self, can_be_deleted=True):
        ...
        records_to_exclude = self.env['calendar.schedule']
        records_to_unlink = self.env['calendar.schedule'].with_context(recompute=False)
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        ...
        if not self._context.get('virtual_id', virtual_id_fallback):
            return super(Meeting, self)._search(new_args, offset=offset, limit=limit, order=order, count=count,
        ...
        event_ids = super(Meeting, self)._search(new_args, offset=0, limit=0, order=None, count=False,
                                                 access_rights_uid=access_rights_uid)
